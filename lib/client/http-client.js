...
const { PrefillAPIError, PrefillResourceNotFoundError,
    PrefillRequestValidationError, PrefillAuthenticationError } = require('../errors');
    ...
    return fetch(url, options).then(result => {
        ...
        switch (contentType) {
            ...
            default: {
                ...
                throw new PrefillAPIError('Invalid response from API');
            }
        }
    }).catch(e => {
        throw new PrefillAPIError(e);
    }).then(body => {
...
function handleHttpStatusCodes(responseBody, httpResponse) {
    ...
    switch (true) {
        ...
        case status === 404: {
            throw new PrefillResourceNotFoundError(message);
        }
        case status === 400: {
            throw new PrefillRequestValidationError(message);
        }
        case status === 401: {
            throw new PrefillAuthenticationError(message);
        }
        default: {
            throw new PrefillAPIError(message);
        }
    }
}
