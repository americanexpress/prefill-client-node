...
const { PrefillRequestValidationError } = require('../../lib/errors');
...
test('should return validation exception', () => {
    function getBearerToken() {
        httpclient.callService.mockImplementation(() => {
            throw new PrefillRequestValidationError()
        });
        return sdk.authentication.getBearerToken();
    }
    expect(getBearerToken).toThrow(PrefillRequestValidationError);
});